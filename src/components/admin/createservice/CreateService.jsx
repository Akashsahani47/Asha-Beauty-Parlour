...
const CreateService = () => {
  ...
  const handleImageUpload = (e) => {
    ...
    // In a real app, you would upload these to cloud storage and get URLs
  ...
  const categories = ['HairCut', 'Skin', 'Makeup', 'Nails', 'Spa', 'Bridal','Facial','Bleach', 'HairStyle','eyebrows','Other']
      ...
      <div className="max-w-4xl mx-auto px-3 lg:px-4">
        ...
        <div className="text-center mb-6 lg:mb-12">
          ...
          <p className="text-[#413329]/70 text-sm lg:text-lg px-2">
            Add a new beauty service to your portfolio
          </p>
        </div>
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-2xl lg:rounded-3xl shadow-2xl border border-white/40 p-4 lg:p-8">
          <form onSubmit={handleSubmit} className="space-y-6 lg:space-y-8">
            ...
            <div>
              <label className="block text-base lg:text-lg font-semibold text-[#413329] mb-3 lg:mb-4 flex items-center">
                <Upload className="w-5 h-5 lg:w-6 lg:h-6 mr-2 lg:mr-3 text-[#413329]/60" />
                Service Images
              </label>
              <div className="border-2 border-dashed border-[#413329]/20 rounded-xl lg:rounded-2xl p-4 lg:p-6 text-center hover:border-[#413329]/40 transition-all duration-300">
                <input
                  type="file"
                  multiple
                  accept="image/*"
                ...
                >
                  <Upload className="w-8 h-8 lg:w-12 lg:h-12 text-[#413329]/40 mx-auto mb-2 lg:mb-4" />
                  <p className="text-[#413329]/70 text-sm lg:text-base">
                    Click to upload images or drag and drop
                  </p>
                  <p className="text-[#413329]/50 text-xs lg:text-sm mt-1">
                  ...
                  {formData.images.map((image, index) => (
                    <div key={index} className="relative group">
                      <img
                        src={image}
                        alt={`Preview ${index + 1}`}
                        className="w-full h-20 lg:h-24 object-cover rounded-xl lg:rounded-2xl"
                      />
                      <button
                        type="button"
                        onClick={() => removeImage(index)}
                        className="absolute -top-2 -right-2 w-6 h-6 lg:w-8 lg:h-8 bg-rose-500 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform hover:scale-110"
                      >
